# Chương trình giải mã Caesar
#
# Yêu cầu:
# - Viết chương trình giải mã Caesar.
...
def dich_ky_tu(ky_tu, do_dich):
    ...
    if "A" <= ky_tu <= "Z":
        return chr((ord(ky_tu) - ord("A") + do_dich) % 26 + ord("A"))
    return ky_tu
chuoi = input("Nhập chuỗi cần giải mã: ")
do_dich = int(input("Nhập độ dịch đã dùng khi mã hóa: "))
ket_qua = "".join(dich_ky_tu(ky_tu, -do_dich) for ky_tu in chuoi)
print("Chuỗi sau giải mã:", ket_qua)
